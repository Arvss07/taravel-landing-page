        ...
        <div className="team-grid grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-6 lg:gap-8 mb-16 max-w-8xl mx-auto">
          {teamMembers.map((member) => (
            <div key={member.id} className="team-card group">
              <div
                className={`card bg-base-100 shadow-lg hover:shadow-xl transition-all duration-300 h-full border border-blue-200 hover:border-opacity-50`}
              >
                <div className="card-body p-6 text-center grid grid-rows-[auto_1fr_auto_auto] gap-4 h-full min-h-[320px]">
                  {/* Row 1 - Picture */}
                  <div className="flex items-center justify-center">
                    <div
                      className={`w-20 h-20 sm:w-24 sm:h-24 lg:w-28 lg:h-28 bg-blue-50 rounded-full flex items-center justify-center group-hover:scale-110 transition-transform duration-300`}
                    >
                      <Image
                        src={member.image}
                        alt={member.name}
